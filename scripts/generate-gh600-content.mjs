...
const questionSpecs = {
  ...
  D4: [
    ...
    },
    {
      ...
      trap: "只重跑先前失敗的案例很常見，但修好那一個，不代表沒有讓其他案例變差。",
      sources: ["GH600-SG"],
    },
    {
      type: "single",
      ...
      trap: "評估工具的預設指標很誘人：現成指標很方便，但方便量測不等於量到你要的東西。",
      sources: ["GH600-SG"],
    },
    {
      type: "single",
  ...
  D5: [
    ...
    },
    {
      ...
      trap: "「先合併再 rebase」是常見的 Git 習慣，但它處理的是文字衝突，沒有處理兩邊的意圖是否矛盾。",
      sources: ["GH600-SG"],
    },
    {
      type: "single",
    ...
    },
    {
      ...
      trap: "「在組織層級建立同名 agent」利用了層級覆蓋的概念，但方向相反：repository 層級優先於組織與企業層級。",
      sources: ["GH-DOC-06", "GH-DOC-31"],
    },
    {
      type: "single",
  ...
  D6: [
    {
      ...
      objective: "D6-O1",
      question:
        "團隊的分級規則是：出錯後無法靠 revert PR 還原的動作才需要人工核准，其餘交給 CI 與 review。下列哪些動作需要人工核准？",
      options: o(
        "執行會刪除正式資料列的資料遷移",
      ...
      answer: ["A", "C"],
      explanation:
        "D6-O1 要求依風險分類代理的動作，決定人工介入的程度；這題的分級規則是「能不能用 revert 還原」。刪除的資料列不會因為 revert 遷移檔而回來，寄出的信也收不回，兩者都需要人工核准。workflow 排程與樣式表都存在 repository 中，出錯時 revert PR 再部署即可還原，交給 CI 與 review 把關就夠了。",
      trap: "「正式環境」四個字不等於高風險。依題幹規則，要問的是出錯後能不能還原，不是動作聽起來多重要。",
      sources: ["GH600-SG", "GH-DOC-30"],
    },
    {
      type: "multiple",
    ...
    },
    {
      ...
      objective: "D6-O2",
      question:
        "cloud agent 的流程中有四個人工關卡。依 D6-O2，哪一個是實質降低風險、應該保留的？",
      options: o(
        "代理讀取 repo 檔案前需要人工確認",
        "代理在自己的分支 commit 前需要確認",
        "Copilot PR 的 workflow 執行前需要核准",
        "代理建立 draft PR 前需要人工確認",
      ),
      answer: "C",
      explanation:
        "cloud agent 開的 PR 上，workflow 預設不會自動執行，要等有寫入權的使用者審查程式碼、按下 Approve and run workflows 才會執行，避免未經人看過的程式碼自動觸發 workflow 並使用其中的 secrets 與權限。讀取檔案不改變任何狀態；代理只能推送到自己的分支；draft PR 在合併前還要經過審查，這三個關卡都不改變結果，只增加等待。",
    ...
    },
    {
      ...
      objective: "D6-O2",
      question:
        "你要確保 .github/agents/ 的任何修改都必須經平台團隊核准才能合併，不論修改者是人還是代理。應該設定什麼？",
      options: o(
        "CODEOWNERS 指定平台團隊，並要求 Code Owners 審查",
        "ruleset 要求合併前至少取得一位 reviewer 核准",
        "preToolUse hook 拒絕代理編輯這個目錄",
        "在 custom agent 的 tools 移除 edit 別名",
      ),
      answer: "A",
      explanation:
        "GitHub 建議用 CODEOWNERS 搭配 ruleset 的「Require review from Code Owners」保護 Copilot 與 agent 設定檔：指定平台團隊為擁有者後，這個目錄的任何變更都必須由他們核准。一般的核准規則任何 reviewer 都能滿足，不限平台團隊；hook 與 tools 只約束代理，擋不住人直接修改。",
      trap: "題幹有兩個條件：核准者限定平台團隊，對象包含人與代理。只約束代理的控制，或不限核准者的控制，都只滿足一半。",
    ...
    },
    {
      ...
      objective: "D6-O1",
      question:
        "要調整正式環境服務帳號的權限，事後必須能回答「誰核准、誰執行、改了什麼」。哪一種做法能留下這三項紀錄？",
      options: o(
        "代理開 PR 修改 IaC 檔，核准後由 workflow 套用",
        "代理在 session 中直接呼叫雲端 CLI 套用變更",
        "reviewer 在聊天中同意後，代理以管理員權杖套用",
        "代理先套用到 staging，成功後自動套用到正式環境",
      ),
      answer: "A",
      explanation:
        "敏感變更要走受控路徑，讓授權、執行、驗證與稽核都能追溯。PR 記錄了改了什麼與誰核准，workflow 的執行紀錄說明誰在何時套用。直接用 CLI 套用只有 session log，沒有核准紀錄；聊天中的同意不是可稽核的核准；staging 成功後自動套用到正式環境，則完全沒有人工核准。",
      trap: "session log 能說明代理做了什麼，但不能證明有人核准。三項紀錄要同時存在，缺一項就無法稽核。",
      sources: ["GH600-SG", "GH-DOC-30"],
    },
  ],
};
